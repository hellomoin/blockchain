                                ...
                                requireIsOperational
    {
        for (uint256 c = 0; c < passengerAddresses.length; c++) {
            if(passengers[passengerAddresses[c]].boughtFlight[flightCode] != 0) {
                ...
                passengers[passengerAddresses[c]].boughtFlight[flightCode] = 0; // First set to 0

                passengers[passengerAddresses[c]].credit = savedCredit + payedPrice + payedPrice.div(2); // paid price = paid price * 1.5
            }
        }
    }
