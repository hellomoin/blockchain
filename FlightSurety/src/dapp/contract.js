...
export default class Contract {
    ...
    async initialize(callback) {
        ...
        this.web3.eth.getAccounts((error, accounts) => {
            ...
            this.flightSuretyData.methods.authorizeCaller(this.appAddress).send({from: this.owner}, (error, result) => {
                if(error) {
                    console.log("failed to authorize the app contract");
                    console.log(error);
                }
            });
                ...
                if (error) {
                    ...
                    isRegistered(payload.airlineAddress).call({ from: payload.sender}, (error, result) => {
                        if (error || result.toString() === 'false') {
                            payload.message = 'New airline needs at least 4 votes for registeration.';
                            payload.registered = false;
                            callback(error, payload);
                        } else {
                            payload.message = 'Airline registered ' + payload.name + ' having address ' + payload.airlineAddress;
    ...
    async buy(flight, price, callback) {
        ...
        self.flightSuretyData.methods.buy(flight)
                                    .send({from: payload.passenger, value: priceInWei, gas: 500000, gasPrice: 1}, (error, result) => {
                                            callback(error, payload);
                                        });
    }
    ...
    async getCreditToPay(callback) {
        ...
        self.flightSuretyData.methods.getCreditToPay().call({from: self.accounts[0]}, (error, result) => {
            callback(error, result);
        });
    }
    ...
    async pay(callback) {
        ...
        self.flightSuretyData.methods.pay(self.accounts[0]).send({from: self.accounts[0]}, (error, result) => {
            callback(error, result);
        });
    }
}